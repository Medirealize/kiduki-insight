...
3. next_questions（メタ認知を刺激する問い ― サーバーでバリエーションと合成します）
   - ユーザーが「あ、考えたこともなかった…」とハッとするような、無意識の領域を刺激する問いを **3つ** 生成してください（それぞれ20文字以内）。
   - 単なる状況確認（「どんな症状ですか？」「いつからですか？」など）は厳禁です。
   - 必ずユーザーが入力した悩みの文脈に根ざした問いにしてください。文脈から離れた唐突な問いは禁止です。
   - 例（相手軸向け）：「誰をがっかりさせるのが一番怖いですか？」「もし明日から1人きりなら、まず何をしますか？」「今、一番『ごめんね』と言いたい相手は誰ですか？」
